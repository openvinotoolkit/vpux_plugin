...
static InferenceParams adjustParams(OpenVINOParams&& params, const GlobalOptions& opts) {
    ...
    // NB: Fill the device property if not specified yet
...
static InferenceParams adjustParams(InferenceParams&& params, const GlobalOptions& opts) {
    if (std::holds_alternative<OpenVINOParams>(params)) {
        return adjustParams(std::get<OpenVINOParams>(std::move(params)), opts);
    }
...
static StreamDesc parseStream(const YAML::Node& node, const GlobalOptions& opts, const std::string& default_name) {
    ...
    for (auto& networks : networks_list) {
        for (auto& network : networks) {
            ...
            stream.infer_params_map.emplace(network.tag, adjustParams(std::move(network.params), opts));
        }
    }
    return stream;
}
...
static std::vector<StreamDesc> parseStreams(const YAML::Node& node, const GlobalOptions& opts) {
    ...
    for (const auto& subnode : node) {
        ...
        auto stream = subnode["op_desc"] ? parseAdvancedStream(subnode, opts, default_name)
                                         : parseStream(subnode, opts, default_name);
...
static std::vector<ScenarioDesc> parseScenarios(const YAML::Node& node, const GlobalOptions& opts) {
    ...
    for (const auto& subnode : node) {
        ...
        scenario.streams = parseStreams(subnode["input_stream_list"], opts);
...
Config parseConfig(const YAML::Node& node) {
    ...
    Config config;
    config.scenarios = parseScenarios(node["multi_inference"], global_opts);
