...
//CHECK: VPUASM.DPUInvariant_37XX @[[SYMINV0:.*]] idx(!VPURegMapped.Index<0:0:0>) taskLocation(@[[TBIVAR0]])
    // CHECK-SAME: input(@[[SYMBUFF2]]) weight_table(@[[SYMBUFF7]]) parent_input(@[[SYMBUFF4]])
    // CHECK-SAME: parent_output(@[[SYMBUFF5]]) outputs([@[[SYMBUFF3]]]) waits([]) updates([])

//CHECK: VPUASM.DPUVariant_37XX @[[SYMVAR0:.*]] idx(!VPURegMapped.Index<0:0:0>) taskLocation(@[[TBVAR0]])
    // CHECK-SAME: calls @[[SYMINV0]]

//CHECK: VPUASM.MappedInference_37XX @MappedInference : invariants(@[[SYMINV0]]) variants(@[[SYMVAR0]])
