...
#NHWC = affine_map<(d0, d1, d2, d3) -> (d0, d2, d3, d1)>
module @TestDMLUnrollDistributedBuffer {

IE.CNNNetwork entryPoint : @unroll_distributed_buff inputsInfo : {
    DataInfo "input_0" : tensor<1x16x16x16xf16>
    } outputsInfo : {
    DataInfo "output_0" : tensor<64x32x1x1xf16>
    }

func.func @unroll_distributed_buff(%arg0: memref<1x16x16x16xf16, @DDR>, %arg1: memref<64x32x1x1xf16, @DDR>) -> memref<64x32x1x1xf16, @DDR> {
  %cst = const.Declare memref<64x32x1x1xf16, {compression = #VPUIP.Compression<CompiletimeCompressed>, order = #NHWC}, @DDR> = dense<1.000000e+00> : tensor<64x32x1x1xf16>, [#const.Reorder<#NHWC>]

  %output_buffer = VPURT.DeclareBuffer <CMX_NN> [0, 1] <0> {DescriptorHandle = 4 : ui64} -> !VPUIP.DistributedBuffer<64x32x1x1xf16, {order = #NHWC, strides = [32, 1, 32, 32]}, @CMX_NN, {mode = "DUPLICATED", num_clusters = 2 : i64, uniform_distributed_segments}>

  VPURT.Task attributes {isTrailingSWLayer = false} {
    %res = VPUIP.DecompressDMAOp {port = 0 : i64}
    inputs(%cst : memref<64x32x1x1xf16, {compression = #VPUIP.Compression<CompiletimeCompressed>, order = #NHWC}, @DDR>)
    outputs(%output_buffer : !VPUIP.DistributedBuffer<64x32x1x1xf16, {order = #NHWC, strides = [32, 1, 32, 32]}, @CMX_NN, {mode = "DUPLICATED", num_clusters = 2 : i64, uniform_distributed_segments}>) -> !VPUIP.DistributedBuffer<64x32x1x1xf16, {order = #NHWC, strides = [32, 1, 32, 32]}, @CMX_NN, {mode = "DUPLICATED", num_clusters = 2 : i64, uniform_distributed_segments}>
  }
