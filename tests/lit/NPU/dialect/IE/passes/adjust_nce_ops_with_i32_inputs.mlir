...
func.func @ConvSI32toFP16(%arg0: tensor<1x1x128x64xsi32, {order = #NHWC}>) -> tensor<1x2x64x64xsi32, {order = #NHWC}> {
    ...
    // CHECK-DAG:   [[FILTERS:%.*]] = const.Declare tensor<2x1x2x1xf16, {order = #NHWC}> = dense<1> : tensor<2x1x2x1xsi32, {order = #NHWC}>, [#const.ConvertElemType<f16>]
...
func.func @GroupConvSI32toFP16(%arg0: tensor<1x16x300x300xsi32>) -> tensor<1x16x300x300xsi32> {
    ...
    // CHECK-DAG:   [[FILTERS:%.*]] = const.Declare tensor<16x1x3x3xf16> = dense<1> : tensor<16x1x1x3x3xsi32>, [#const.Reshape<[16, 1, 3, 3]>, #const.ConvertElemType<f16>]
