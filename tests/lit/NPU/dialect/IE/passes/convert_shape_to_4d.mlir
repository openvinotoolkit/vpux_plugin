...
func.func @Convert3dAddWithSecondDim(%arg0: tensor<1x80x1xf16>) -> tensor<1x80x1xf16> {
    ...
    // CHECK-SAME:  dense<2.000000e+00> : tensor<1x80x1xf16>, [#const.Reshape<[1, 1, 80, 1]>, #const.Reshape<[1, 80, 1, 1]>]
...
func.func @Convert2dAddWithLastDim(%arg0: tensor<1x80xf16>) -> tensor<1x80xf16> {
    ...
    // CHECK-DAG:   [[ADD_WEIGHTS:%.*]] = const.Declare tensor<1x80x1x1xf16> =
    // CHECK-SAME:  dense<2.000000e+00> : tensor<1x80xf16>, [#const.Reshape<[1, 1, 1, 80]>, #const.Reshape<[1, 80, 1, 1]>]
...
func.func @ConvertBroadcastSubgraph(%input: tensor<1x4x1024x64xf16>) -> tensor<1x32x1024x64xf16> {
    ...
    // CHECK:   [[CST:%.*]] = const.Declare tensor<4xsi64> = dense<[4, 8, 1024, 64]>
    ...
    // CHECK-SAME:     : tensor<4x1x1024x64xf16>, tensor<4xsi64> -> tensor<4x8x1024x64xf16>
...
func.func @ConvertBroadcast(%input: tensor<1x4x1x1024x64xf16>) -> tensor<1x4x16x1024x64xf16> {
    ...
    // CHECK:   [[CST:%.*]] = const.Declare tensor<4xsi64> = dense<[4, 16, 1024, 64]>
    // CHECK-SAME:      : tensor<4xsi64>, [#const.ConvertElemType<si32>]
    ...
    // CHECK:   [[BROADCAST:%.*]] = IE.Broadcast([[IN_RESHAPE]], [[CST]])
    // CHECK-SAME:      {mode = #IE.broadcast_type<BIDIRECTIONAL>}
    // CHECK-SAME:      : tensor<4x1x1024x64xf16>, tensor<4xsi64> -> tensor<4x16x1024x64xf16>
