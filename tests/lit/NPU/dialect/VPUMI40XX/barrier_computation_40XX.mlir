...
func.func @main(%arg0: memref<1x1x2x1000xf16>, %arg1: memref<1x1x2x1000xf16>) -> memref<1x1x2x1000xf16> {
    ...
    // CHECK:       %[[VAL0:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 0 : ui8} <0, -1> -> !VPURegMapped.Index<0:0:0>
    ...
    // CHECK:       %[[VAL3:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <3, -1> -> !VPURegMapped.Index<0:0:3>

    %barrier4 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <4,-1> -> !VPURegMapped.Index<0:0:4>

    // CHECK:       %[[VAL4:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <4, -1> -> !VPURegMapped.Index<0:0:4>

    %barrier5 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <5,-1> -> !VPURegMapped.Index<0:0:5>

    // CHECK:       %[[VAL5:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <5, -1> -> !VPURegMapped.Index<0:0:5>

    %barrier6 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <6,-1> -> !VPURegMapped.Index<0:0:6>

    // CHECK:       %[[VAL6:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <6, -1> -> !VPURegMapped.Index<0:0:6>

    %barrier7 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <7,-1> -> !VPURegMapped.Index<0:0:7>

    // CHECK:       %[[VAL7:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <7, -1> -> !VPURegMapped.Index<0:0:7>

    %barrier8 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <8,-1> -> !VPURegMapped.Index<0:0:8>

    // CHECK:       %[[VAL8:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <8, -1> -> !VPURegMapped.Index<0:0:8>

    %barrier9 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <9,-1> -> !VPURegMapped.Index<0:0:9>

    // CHECK:       %[[VAL9:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <9, -1> -> !VPURegMapped.Index<0:0:9>

    %barrier10 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <10,-1> -> !VPURegMapped.Index<0:0:10>

    // CHECK:       %[[VAL10:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <10, -1> -> !VPURegMapped.Index<0:0:10>

    %barrier11 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <11,-1> -> !VPURegMapped.Index<0:0:11>

    // CHECK:       %[[VAL11:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <11, -1> -> !VPURegMapped.Index<0:0:11>

    %barrier12 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <12,-1> -> !VPURegMapped.Index<0:0:12>

    // CHECK:       %[[VAL12:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <12, -1> -> !VPURegMapped.Index<0:0:12>

    %barrier13 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <13,-1> -> !VPURegMapped.Index<0:0:13>

    // CHECK:       %[[VAL13:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <13, -1> -> !VPURegMapped.Index<0:0:13>

    %barrier14 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <14,-1> -> !VPURegMapped.Index<0:0:14>

    // CHECK:       %[[VAL14:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <14, -1> -> !VPURegMapped.Index<0:0:14>

    %barrier15 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <15,-1> -> !VPURegMapped.Index<0:0:15>

    // CHECK:       %[[VAL15:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <15, -1> -> !VPURegMapped.Index<0:0:15>

    %barrier16 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <16,-1> -> !VPURegMapped.Index<0:0:16>

    // CHECK:       %[[VAL16:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <16, -1> -> !VPURegMapped.Index<0:0:16>

    %barrier17 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <17,-1> -> !VPURegMapped.Index<0:0:17>

    // CHECK:       %[[VAL17:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <17, -1> -> !VPURegMapped.Index<0:0:17>

    %barrier18 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <18,-1> -> !VPURegMapped.Index<0:0:18>

    // CHECK:       %[[VAL18:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <18, -1> -> !VPURegMapped.Index<0:0:18>

    %barrier19 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <19,-1> -> !VPURegMapped.Index<0:0:19>

    // CHECK:       %[[VAL19:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <19, -1> -> !VPURegMapped.Index<0:0:19>

    %barrier20 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <20,-1> -> !VPURegMapped.Index<0:0:20>

    // CHECK:       %[[VAL20:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <20, -1> -> !VPURegMapped.Index<0:0:20>

    %barrier21 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <21,-1> -> !VPURegMapped.Index<0:0:21>

    // CHECK:       %[[VAL21:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <21, -1> -> !VPURegMapped.Index<0:0:21>

    %barrier22 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <22,-1> -> !VPURegMapped.Index<0:0:22>

    // CHECK:       %[[VAL22:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <22, -1> -> !VPURegMapped.Index<0:0:22>

    %barrier23 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <23,-1> -> !VPURegMapped.Index<0:0:23>

    // CHECK:       %[[VAL23:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <23, -1> -> !VPURegMapped.Index<0:0:23>

    %barrier24 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <24,-1> -> !VPURegMapped.Index<0:0:24>

    // CHECK:       %[[VAL24:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <24, -1> -> !VPURegMapped.Index<0:0:24>

    %barrier25 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <25,-1> -> !VPURegMapped.Index<0:0:25>

    // CHECK:       %[[VAL25:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <25, -1> -> !VPURegMapped.Index<0:0:25>

    %barrier26 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <26,-1> -> !VPURegMapped.Index<0:0:26>

    // CHECK:       %[[VAL26:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <26, -1> -> !VPURegMapped.Index<0:0:26>

    %barrier27 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <27,-1> -> !VPURegMapped.Index<0:0:27>

    // CHECK:       %[[VAL27:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <27, -1> -> !VPURegMapped.Index<0:0:27>

    %barrier28 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <28,-1> -> !VPURegMapped.Index<0:0:28>

    // CHECK:       %[[VAL28:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <28, -1> -> !VPURegMapped.Index<0:0:28>

    %barrier29 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <29,-1> -> !VPURegMapped.Index<0:0:29>

    // CHECK:       %[[VAL29:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <29, -1> -> !VPURegMapped.Index<0:0:29>

    %barrier30 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <30,-1> -> !VPURegMapped.Index<0:0:30>

    // CHECK:       %[[VAL30:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <30, -1> -> !VPURegMapped.Index<0:0:30>

    %barrier31 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <31,-1> -> !VPURegMapped.Index<0:0:31>

    // CHECK:       %[[VAL31:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <31, -1> -> !VPURegMapped.Index<0:0:31>

    %barrier32 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <0,-1> -> !VPURegMapped.Index<0:0:32>

    // CHECK:       %[[VAL32:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <0, -1> -> !VPURegMapped.Index<0:0:32>

    %barrier33 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <1,-1> -> !VPURegMapped.Index<0:0:33>

    // CHECK:       %[[VAL33:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <1, -1> -> !VPURegMapped.Index<0:0:33>

    %barrier34 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <2,-1> -> !VPURegMapped.Index<0:0:34>

    // CHECK:       %[[VAL34:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <2, -1> -> !VPURegMapped.Index<0:0:34>

    %barrier35 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <3,-1> -> !VPURegMapped.Index<0:0:35>

    // CHECK:       %[[VAL35:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <3, -1> -> !VPURegMapped.Index<0:0:35>

    %barrier36 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <4,-1> -> !VPURegMapped.Index<0:0:36>

    // CHECK:       %[[VAL36:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <4, -1> -> !VPURegMapped.Index<0:0:36>

    %barrier37 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <5,-1> -> !VPURegMapped.Index<0:0:37>

    // CHECK:       %[[VAL37:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <5, -1> -> !VPURegMapped.Index<0:0:37>

    %barrier38 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <6,-1> -> !VPURegMapped.Index<0:0:38>

    // CHECK:       %[[VAL38:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <6, -1> -> !VPURegMapped.Index<0:0:38>

    %barrier39 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <7,-1> -> !VPURegMapped.Index<0:0:39>

    // CHECK:       %[[VAL39:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <7, -1> -> !VPURegMapped.Index<0:0:39>

    %barrier40 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <8,-1> -> !VPURegMapped.Index<0:0:40>

    // CHECK:       %[[VAL40:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <8, -1> -> !VPURegMapped.Index<0:0:40>

    %barrier41 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <9,-1> -> !VPURegMapped.Index<0:0:41>

    // CHECK:       %[[VAL41:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <9, -1> -> !VPURegMapped.Index<0:0:41>

    %barrier42 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <10,-1> -> !VPURegMapped.Index<0:0:42>

    // CHECK:       %[[VAL42:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <10, -1> -> !VPURegMapped.Index<0:0:42>

    %barrier43 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <11,-1> -> !VPURegMapped.Index<0:0:43>

    // CHECK:       %[[VAL43:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <11, -1> -> !VPURegMapped.Index<0:0:43>

    %barrier44 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <12,-1> -> !VPURegMapped.Index<0:0:44>

    // CHECK:       %[[VAL44:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <12, -1> -> !VPURegMapped.Index<0:0:44>

    %barrier45 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <13,-1> -> !VPURegMapped.Index<0:0:45>

    // CHECK:       %[[VAL45:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <13, -1> -> !VPURegMapped.Index<0:0:45>

    %barrier46 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <14,-1> -> !VPURegMapped.Index<0:0:46>

    // CHECK:       %[[VAL46:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <14, -1> -> !VPURegMapped.Index<0:0:46>

    %barrier47 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <15,-1> -> !VPURegMapped.Index<0:0:47>

    // CHECK:       %[[VAL47:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <15, -1> -> !VPURegMapped.Index<0:0:47>

    %barrier48 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <16,-1> -> !VPURegMapped.Index<0:0:48>

    // CHECK:       %[[VAL48:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <16, -1> -> !VPURegMapped.Index<0:0:48>

    %barrier49 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <17,-1> -> !VPURegMapped.Index<0:0:49>

    // CHECK:       %[[VAL49:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <17, -1> -> !VPURegMapped.Index<0:0:49>

    %barrier50 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <18,-1> -> !VPURegMapped.Index<0:0:50>

    // CHECK:       %[[VAL50:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <18, -1> -> !VPURegMapped.Index<0:0:50>

    %barrier51 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <19,-1> -> !VPURegMapped.Index<0:0:51>

    // CHECK:       %[[VAL51:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <19, -1> -> !VPURegMapped.Index<0:0:51>

    %barrier52 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <20,-1> -> !VPURegMapped.Index<0:0:52>

    // CHECK:       %[[VAL52:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <20, -1> -> !VPURegMapped.Index<0:0:52>

    %barrier53 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <21,-1> -> !VPURegMapped.Index<0:0:53>

    // CHECK:       %[[VAL53:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <21, -1> -> !VPURegMapped.Index<0:0:53>

    %barrier54 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <22,-1> -> !VPURegMapped.Index<0:0:54>

    // CHECK:       %[[VAL54:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <22, -1> -> !VPURegMapped.Index<0:0:54>

    %barrier55 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <23,-1> -> !VPURegMapped.Index<0:0:55>

    // CHECK:       %[[VAL55:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <23, -1> -> !VPURegMapped.Index<0:0:55>

    %barrier56 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <24,-1> -> !VPURegMapped.Index<0:0:56>

    // CHECK:       %[[VAL56:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <24, -1> -> !VPURegMapped.Index<0:0:56>

    %barrier57 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <25,-1> -> !VPURegMapped.Index<0:0:57>

    // CHECK:       %[[VAL57:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <25, -1> -> !VPURegMapped.Index<0:0:57>

    %barrier58 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <26,-1> -> !VPURegMapped.Index<0:0:58>

    // CHECK:       %[[VAL58:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <26, -1> -> !VPURegMapped.Index<0:0:58>

    %barrier59 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <27,-1> -> !VPURegMapped.Index<0:0:59>

    // CHECK:       %[[VAL59:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <27, -1> -> !VPURegMapped.Index<0:0:59>

    %barrier60 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <28,-1> -> !VPURegMapped.Index<0:0:60>

    // CHECK:       %[[VAL60:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <28, -1> -> !VPURegMapped.Index<0:0:60>

    %barrier61 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <29,-1> -> !VPURegMapped.Index<0:0:61>

    // CHECK:       %[[VAL61:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <29, -1> -> !VPURegMapped.Index<0:0:61>

    %barrier62 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <30,-1> -> !VPURegMapped.Index<0:0:62>

    // CHECK:       %[[VAL62:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <30, -1> -> !VPURegMapped.Index<0:0:62>

    %barrier63 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <31,-1> -> !VPURegMapped.Index<0:0:63>

    // CHECK:       %[[VAL63:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <31, -1> -> !VPURegMapped.Index<0:0:63>

    %barrier64 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <0,-1> -> !VPURegMapped.Index<0:0:64>

    // CHECK:       %[[VAL64:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <0, -1> -> !VPURegMapped.Index<0:0:64>

    %barrier65 = VPUMI40XX.ConfigureBarrier {consumer_count=1:ui8, producer_count=1:ui8 } <1,-1> -> !VPURegMapped.Index<0:0:65>

    // CHECK:       %[[VAL65:.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <1, -1> -> !VPURegMapped.Index<0:0:65>
