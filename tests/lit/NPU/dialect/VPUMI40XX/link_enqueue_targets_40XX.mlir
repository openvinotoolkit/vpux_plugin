...
// RUN: vpux-opt --split-input-file --vpu-arch=%arch% --link-enqueue-targets %s | FileCheck %s
...
//CHECK: [[DMA1:%.*]] = VPUMI40XX.NNDMA {port = 0 : i64} {{.*}} -> !VPURegMapped.Index<0:0:1>
//CHECK: [[DMA2:%.*]] = VPUMI40XX.NNDMA {HardLinkedAttrName, port = 0 : i64} {{.*}} -> !VPURegMapped.Index<0:0:2>
//CHECK: [[DMA3:%.*]] = VPUMI40XX.NNDMA {HardLinkedAttrName, port = 0 : i64} {{.*}} -> !VPURegMapped.Index<0:0:3>
//CHECK: [[DMA4:%.*]] = VPUMI40XX.NNDMA {port = 0 : i64} {{.*}} -> !VPURegMapped.Index<0:0:4>
//CHECK: [[DMA5:%.*]] = VPUMI40XX.NNDMA {HardLinkedAttrName, port = 0 : i64} {{.*}} -> !VPURegMapped.Index<0:0:5>

//CHECK: [[BARR0:%.*]] = VPUMI40XX.ConfigureBarrier {consumer_count = 1 : ui8, producer_count = 1 : ui8} <4, -1> -> !VPURegMapped.Index<0:0:0>

//CHECK: VPURegMapped.Enqueue at([[BARR0]] : !VPURegMapped.Index<0:0:0>) ([[DMA1]] -> [[DMA1]] : <0:0:1> -> <0:0:1>) -> !VPURegMapped.Index<0:0:0> {taskType = #VPURegMapped.task_type<DPUVariant>}
//CHECK: VPURegMapped.Enqueue at([[BARR0]] : !VPURegMapped.Index<0:0:0>) ([[DMA4]] -> [[DMA4]] : <0:0:4> -> <0:0:4>) -> !VPURegMapped.Index<0:0:1> {taskType = #VPURegMapped.task_type<DPUVariant>}
