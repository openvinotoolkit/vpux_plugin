...
func.func @BuildSubgraphConvSwishGroupConvVF(%arg0: tensor<1x16x176x176xf16, {order = #NHWC}>) -> tensor<1x96x176x176xf16, {order = #NHWC}> {
    ...
    //CHECK-SAME:                        %cst_1 as %arg4: tensor<1x1x1x16xui8>)
...
func.func @BuildSubgraphConvGeluVF(%arg0: tensor<1x16x128x128xf16, {order = #NHWC}>) -> tensor<1x64x128x128xf16, {order = #NHWC}> {
    %cst_0 = const.Declare tensor<64x16x1x1xf16, {order = #NHWC}> = dense<1.0> : tensor<64x16xf32>, [#const.Reshape<[64, 16, 1, 1]>, #const.ConvertElemType<f16>, #const.Reorder<#NHWC>]
    ...
    //CHECK-DAG: [[WEIGHTS:%.+]] = const.Declare tensor<64x16x1x1xf16, {order = #NHWC}> = dense<1.000000e+00> : tensor<64x16xf32>, [#const.Reshape<[64, 16, 1, 1]>, #const.ConvertElemType<f16>, #const.Reorder<#NHWC>]
    ...
    //CHECK: [[NCE:%.+]] = VPU.NCE.Convolution([[INNER_ARG1]], [[INNER_ARG2]], [[INNER_ARG3]]) {multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPETask<mode = <NOOP>, clamp_low = -2147483648 : i64, clamp_high = 2147483647 : i64, lrelu_mult = 1 : i64, lrelu_shift = 0 : i64, fp_prelu_alpha = 1.000000e+00 : f64>, rawFilterShape = [64, 16, 1, 1], strides = [1, 1]} -> tensor<1x64x128x128xf16, {order = #NHWC}>
...
func.func @NotBuildOutOfLimitNumTiles(%arg0: tensor<1x512x92x120x!qElemType, {order = #NHWC}>) -> tensor<1x256x92x120x!qElemType, {order = #NHWC}> {
    %cst_0 = const.Declare tensor<256x512x3x3x!qElemType, {order = #NHWC}> = dense<1.0> : tensor<256x512x3x3xf16>, [#const.ConvertElemType<ui8>, #const.QuantCast<!qElemType>, #const.Reorder<#NHWC>]
    %cst_1 = const.Declare tensor<256x1x1x4xsi32> = dense<1> : tensor<256x1x1x4xsi32>
    %cst_2 = const.Declare tensor<256x256x3x3x!qElemType, {order = #NHWC}> = dense<1.0> : tensor<256x256x3x3xf16>, [#const.ConvertElemType<ui8>, #const.QuantCast<!qElemType>, #const.Reorder<#NHWC>]
...
func.func @BuildSubgraphConvWithDepthToSpace(%arg0: tensor<1x16x180x270xf16, {order = #NHWC}>) -> tensor<1x1x720x1080xf16, {order = #NHWC}> {
    %cst_0 = const.Declare tensor<16x16x5x5xf16, {order = #NHWC}> = dense<1.0> : tensor<16x16x5x5xf32>, [#const.ConvertElemType<f16>, #const.Reorder<#NHWC>]
...
func.func @BuildSubgraphEltwiseQuantizeCastVF(%arg0: tensor<1x32x48x48x!qElemType, {order = #NHWC}>) -> tensor<1x32x48x48xf16, {order = #NHWC}> {
  %0 = VPU.VerticalFusion (%arg0 as %arg1: tensor<1x32x48x48x!qElemType, {order = #NHWC}>) attributes {tilingStrategy = [1, 1, 2, 1]} -> tensor<1x32x48x48x!qElemType, {order = #NHWC}> {
      %2 = VPU.NCE.Eltwise(%arg1, %arg1)
         {op_type = #VPU.eltwise_type<ADD>, ppe = #VPU.PPETask<mode = <NOOP>, clamp_low = -2147483648 : i64, clamp_high = 2147483647 : i64, lrelu_mult = 1 : i64, lrelu_shift = 0 : i64,
         quant_mult = [16384], quant_shift = [36], quant_post_shift = 0 : i64, in1_quant_mult = [19729], in2_quant_mult = [19729], fp_prelu_alpha = 1.000000e+00 : f64>} -> tensor<1x32x48x48x!qElemType, {order = #NHWC}>
      VPU.Yield %2
  }
...
func.func @NotBuildNotPipelinedSubgraph(%arg0: tensor<1x128x32x64xf16, {order = #NHWC}>) -> tensor<1x64x64x128xf16> {
    ...
    %cst_0 = const.Declare tensor<1x128x67x131xi1, {order = #NHWC}> = dense<1> : tensor<1x128x67x131xi8>, [#const.Reorder<#NHWC>, #const.ConvertElemType<i1>]
    ...
    %0 = VPU.VerticalFusion (%arg0 as %arg1: tensor<1x128x32x64xf16, {order = #NHWC}>, %cst_3 as %arg2: tensor<128x16x1x1xf16, {order = #NHWC}>, %cst_2 as %arg3: tensor<128x1x1x4xsi32>) attributes {tilingStrategy = [1, 1, 1, 1]} -> tensor<1x128x32x64xf16, {order = #NHWC}> {
      %2 = VPU.NCE.DepthConvolution(%arg1, %arg2, %arg3) {multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverKernel>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPETask<mode = <LRELU>, clamp_low = -2147483648 : i64, clamp_high = 2147483647 : i64, lrelu_mult = 1 : i64, lrelu_shift = 0 : i64, fp_prelu_alpha = 1.000000e+00 : f64>, rawFilterShape = [128, 1, 1, 1], strides = [1, 1]} -> tensor<1x128x32x64xf16, {order = #NHWC}>
      VPU.Yield %2
    }
...
func.func @BuildSubgraphConvPReluVF(%arg0: tensor<1x16x128x128xf16, {order = #NHWC}>) -> tensor<1x16x128x128xf16, {order = #NHWC}> {
    %cst_0 = const.Declare tensor<16x16x1x1xf16, {order = #NHWC}> = dense<1.0> : tensor<16x16xf32>, [#const.Reshape<[16, 16, 1, 1]>, #const.ConvertElemType<f16>, #const.Reorder<#NHWC>]
    %cst_1 = const.Declare tensor<16x1x1x4xsi32> = dense<1> : tensor<16x1x1x4xsi32>
    ...
    %0 = VPU.VerticalFusion (%arg0 as %arg2: tensor<1x16x128x128xf16, {order = #NHWC}>, %cst_0 as %arg3: tensor<16x16x1x1xf16, {order = #NHWC}>, %cst_1 as %arg4: tensor<16x1x1x4xsi32>) attributes {tilingStrategy = [1, 1, 2, 1]} -> tensor<1x16x128x128xf16, {order = #NHWC}> {
      %3 = VPU.NCE.Convolution(%arg2, %arg3, %arg4)
        {multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPETask<mode = <NOOP>, clamp_low = -2147483648 : i64, clamp_high = 2147483647 : i64, lrelu_mult = 1 : i64, lrelu_shift = 0 : i64, fp_prelu_alpha = 1.000000e+00 : f64>, rawFilterShape = [16, 16, 1, 1], strides = [1, 1]} -> tensor<1x16x128x128xf16, {order = #NHWC}>
      VPU.Yield %3
    }
    ...
    // CHECK-DAG: [[WEIGHTS:%.+]] = const.Declare tensor<16x16x1x1xf16, {order = #NHWC}> = dense<1.000000e+00> : tensor<16x16xf32>, [#const.Reshape<[16, 16, 1, 1]>, #const.ConvertElemType<f16>, #const.Reorder<#NHWC>]
