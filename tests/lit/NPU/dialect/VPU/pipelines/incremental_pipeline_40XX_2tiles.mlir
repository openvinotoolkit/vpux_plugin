...
func.func @SubgraphIncrementalPipelineCheck(%arg0: tensor<1x16x227x227xf16, {order = #NHWC}>) -> tensor<1x96x111x111xf16, {order = #NHWC}> {
    ...
    // CHECK:       [[COPY_INPUT_0:%.+]] = VPU.NCE.ClusterTiling ([[SLICE_0]] as [[INNER_ARG0:[^:]+]]: tensor<1x16x76x227xf16, {order = #NHWC}>)
    ...
    // CHECK:           VPU.Copy([[INNER_ARG0]]) {out_mem_space = @CMX_NN} : tensor<1x16x76x227xf16, {order = #NHWC}>
    // CHECK-SAME:          -> tensor<1x16x76x227xf16, {mem_space = @CMX_NN, order = #NHWC}>
    // CHECK:           VPU.Yield
    // CHECK:       }
    ...
    // CHECK-SAME:      [[COPY_INPUT_0]] as [[INNER_ARG3:[^:]+]]: tensor<1x16x76x227xf16, {mem_space = @CMX_NN, order = #NHWC}>,
    ...
    // CHECK:       [[COPY_OUT_0:%.+]] = VPU.NCE.ClusterTiling ([[DWCONV_0]] as [[INNER_ARG6:[^:]+]]: tensor<1x16x76x227xf16, {mem_space = @CMX_NN, order = #NHWC}>)
    // CHECK-SAME:      -> tensor<1x16x76x227xf16, {order = #NHWC}> {
    // CHECK:           VPU.Copy([[INNER_ARG6]]) : tensor<1x16x76x227xf16, {mem_space = @CMX_NN, order = #NHWC}> -> tensor<1x16x76x227xf16, {order = #NHWC}>
    // CHECK:           VPU.Yield
    // CHECK:       }
    ...
    // CHECK:       [[COPY_INPUT_1:%.+]] = VPU.NCE.ClusterTiling ([[SLICE_1]] as [[INNER_ARG7:[^:]+]]: tensor<1x16x76x227xf16, {order = #NHWC}>)
    // CHECK-SAME:      -> !VPU.DistributedTensor<1x16x76x227xf16, #NHWC, @CMX_NN,
    // CHECK-SAME:          {mode = "OVERLAPPED", num_tiles = [1, 1, 2, 1], num_clusters = 2 : i64, uniform_distributed_segments
    // CHECK-SAME{LITERAL}:  compute_shapes = [[1, 16, 38, 227], [1, 16, 38, 227]]
    // CHECK-SAME{LITERAL}:  compute_offsets = [[0, 0, 0, 0], [0, 0, 38, 0]]
    // CHECK-SAME{LITERAL}:  memory_shapes = [[1, 16, 38, 227], [1, 16, 38, 227]]
    // CHECK-SAME{LITERAL}:  memory_offsets = [[0, 0, 0, 0], [0, 0, 38, 0]]}> {
    // CHECK:           VPU.Copy([[INNER_ARG7]]) {out_mem_space = @CMX_NN} : tensor<1x16x76x227xf16, {order = #NHWC}>
    // CHECK-SAME:          -> tensor<1x16x76x227xf16, {mem_space = @CMX_NN, order = #NHWC}>
    // CHECK:           VPU.Yield
    // CHECK:       }
    ...
    // CHECK-SAME:      [[COPY_INPUT_1]] as [[INNER_ARG10:[^:]+]]: tensor<1x16x76x227xf16, {mem_space = @CMX_NN, order = #NHWC}>,
    ...
    // CHECK:       [[COPY_OUT_1:%.+]] = VPU.NCE.ClusterTiling ([[DWCONV_1]] as [[INNER_ARG13:[^:]+]]: tensor<1x16x76x227xf16, {mem_space = @CMX_NN, order = #NHWC}>)
    // CHECK-SAME:      -> tensor<1x16x76x227xf16, {order = #NHWC}> {
    // CHECK:           VPU.Copy([[INNER_ARG13]]) : tensor<1x16x76x227xf16, {mem_space = @CMX_NN, order = #NHWC}>
    // CHECK-SAME:          -> tensor<1x16x76x227xf16, {order = #NHWC}>
    // CHECK:           VPU.Yield
    // CHECK:       }
    ...
    // CHECK:       [[COPY_INPUT_2:%.+]] = VPU.NCE.ClusterTiling ([[SLICE_2]] as [[INNER_ARG7:[^:]+]]: tensor<1x16x75x227xf16, {order = #NHWC}>)
    ...
    // CHECK-SAME{LITERAL}:  memory_offsets = [[0, 0, 0, 0], [0, 0, 38, 0]]}> {
    // CHECK:           VPU.Copy([[INNER_ARG7]]) {out_mem_space = @CMX_NN} : tensor<1x16x75x227xf16, {order = #NHWC}>
    // CHECK-SAME:          -> tensor<1x16x75x227xf16, {mem_space = @CMX_NN, order = #NHWC}>
    // CHECK:           VPU.Yield
    // CHECK:       }

    // CHECK:       [[COPY_WEIGHTS_2:%.+]] = VPU.NCE.ClusterTiling ([[WEIGHTS1]] as [[INNER_ARG8:[^:]+]]: tensor<16x16x1x1xf16, {order = #NHWC}>)
    // CHECK-SAME:      -> !VPU.DistributedTensor<16x16x1x1xf16, #NHWC, @CMX_NN,
    // CHECK-SAME:          {mode = "DUPLICATED", num_clusters = 2 : i64, uniform_distributed_segments
    // CHECK-SAME{LITERAL}:  compute_shapes = [[16, 16, 1, 1], [16, 16, 1, 1]]
    // CHECK-SAME{LITERAL}:  compute_offsets = [[0, 0, 0, 0], [0, 0, 0, 0]]
    ...
    // CHECK:       [[COPY_WEIGHTS_TBL_2:%.+]] = VPU.NCE.ClusterTiling ([[WEIGHTS_TABLE1]] as [[INNER_ARG9:[^:]+]]: tensor<16x1x1x4xsi32>)
    // CHECK-SAME:      -> !VPU.DistributedTensor<16x1x1x4xsi32, #NCHW, @CMX_NN,
    // CHECK-SAME:          {mode = "DUPLICATED", num_clusters = 2 : i64, uniform_distributed_segments
    // CHECK-SAME{LITERAL}:  compute_shapes = [[16, 1, 1, 4], [16, 1, 1, 4]]
    // CHECK-SAME{LITERAL}:  compute_offsets = [[0, 0, 0, 0], [0, 0, 0, 0]]
    ...
    // CHECK-SAME:      [[COPY_INPUT_2]] as [[INNER_ARG10:[^:]+]]: tensor<1x16x75x227xf16, {mem_space = @CMX_NN, order = #NHWC}>,
    // CHECK-SAME:      [[COPY_WEIGHTS_2]] as [[INNER_ARG11:[^:]+]]: tensor<16x16x1x1xf16, {mem_space = @CMX_NN, order = #NHWC}>,
    // CHECK-SAME:      [[COPY_WEIGHTS_TBL_2]] as [[INNER_ARG12:[^:]+]]: tensor<16x1x1x4xsi32, {mem_space = @CMX_NN, order = #NCHW}>)
    // CHECK-SAME:          -> !VPU.DistributedTensor<1x16x75x227xf16, #NHWC, @CMX_NN,
    // CHECK-SAME:              {mode = "OVERLAPPED", num_tiles = [1, 1, 2, 1], num_clusters = 2 : i64, uniform_distributed_segments
    // CHECK-SAME{LITERAL}:      compute_shapes = [[1, 16, 38, 227], [1, 16, 37, 227]]
    // CHECK-SAME{LITERAL}:      compute_offsets = [[0, 0, 0, 0], [0, 0, 38, 0]]
    ...
    // CHECK:       [[COPY_OUT_2:%.+]] = VPU.NCE.ClusterTiling ([[DWCONV_2]] as [[INNER_ARG13:[^:]+]]: tensor<1x16x75x227xf16, {mem_space = @CMX_NN, order = #NHWC}>)
    // CHECK-SAME:      -> tensor<1x16x75x227xf16, {order = #NHWC}> {
    // CHECK:           VPU.Copy([[INNER_ARG13]]) : tensor<1x16x75x227xf16, {mem_space = @CMX_NN, order = #NHWC}>
    // CHECK-SAME:          -> tensor<1x16x75x227xf16, {order = #NHWC}>
    // CHECK:           VPU.Yield
    // CHECK:       }
    ...
    // CHECK:       [[COPY_CONV_INPUT_0:%.+]] = VPU.NCE.ClusterTiling ([[SLICE_CONV_0]] as [[INNER_ARG14:[^:]+]]: tensor<1x16x79x227xf16, {order = #NHWC}>)
    ...
    // CHECK:       [[COPY_CONV_WEIGHTS_0:%.+]] = VPU.NCE.ClusterTiling ([[WEIGHTS2]] as [[INNER_ARG15:[^:]+]]: tensor<96x16x7x7xf16, {order = #NHWC}>)
    ...
    // CHECK:       [[COPY_CONV_WEIGHTS_TBL_0:%.+]] = VPU.NCE.ClusterTiling ([[WEIGHTS_TABLE2]] as [[INNER_ARG16:[^:]+]]: tensor<96x1x1x4xsi32>)
    ...
    // CHECK:       [[CONV_0:%.+]] = VPU.NCE.ClusterTiling (
    // CHECK-SAME:      [[COPY_CONV_INPUT_0]] as [[INNER_ARG16:[^:]+]]: tensor<1x16x79x227xf16, {mem_space = @CMX_NN, order = #NHWC}>,
    // CHECK-SAME:      [[COPY_CONV_WEIGHTS_0]] as [[INNER_ARG17:[^:]+]]: tensor<96x16x7x7xf16, {mem_space = @CMX_NN, order = #NHWC}>,
    // CHECK-SAME:      [[COPY_CONV_WEIGHTS_TBL_0]] as [[INNER_ARG18:[^:]+]]: tensor<96x1x1x4xsi32, {mem_space = @CMX_NN, order = #NCHW}>)
    ...
    // CHECK:       [[COPY_CONV_OUT_0:%.+]] = VPU.NCE.ClusterTiling ([[CONV_0]] as [[INNER_ARG19:[^:]+]]: tensor<1x96x37x111xf16, {mem_space = @CMX_NN, order = #NHWC}>)
    ...
    // CHECK:       [[COPY_CONV_INPUT_1:%.+]] = VPU.NCE.ClusterTiling ([[SLICE_CONV_1]] as [[INNER_ARG20:[^:]+]]: tensor<1x16x79x227xf16, {order = #NHWC}>)
    // CHECK-SAME:      -> !VPU.DistributedTensor<1x16x79x227xf16, #NHWC, @CMX_NN,
    // CHECK-SAME:           {mode = "OVERLAPPED", num_tiles = [1, 1, 2, 1], num_clusters = 2 : i64, uniform_distributed_segments
    // CHECK-SAME{LITERAL}:   compute_shapes = [[1, 16, 40, 227], [1, 16, 39, 227]]
    // CHECK-SAME{LITERAL}:   compute_offsets = [[0, 0, 0, 0], [0, 0, 40, 0]]
    ...
    // CHECK:       [[COPY_CONV_WEIGHTS_1:%.+]] = VPU.NCE.ClusterTiling ([[WEIGHTS2]] as [[INNER_ARG21:[^:]+]]: tensor<96x16x7x7xf16, {order = #NHWC}>)
    // CHECK-SAME:    -> !VPU.DistributedTensor<96x16x7x7xf16, #NHWC, @CMX_NN,
    // CHECK-SAME:          {mode = "DUPLICATED", num_clusters = 2 : i64, uniform_distributed_segments
    // CHECK-SAME{LITERAL}:  compute_shapes = [[96, 16, 7, 7], [96, 16, 7, 7]]
    // CHECK-SAME{LITERAL}:  compute_offsets = [[0, 0, 0, 0], [0, 0, 0, 0]]
    ...
    // CHECK:       [[COPY_CONV_WEIGHTS_TBL_1:%.+]] = VPU.NCE.ClusterTiling ([[WEIGHTS_TABLE2]] as [[INNER_ARG22:[^:]+]]: tensor<96x1x1x4xsi32>)
    // CHECK-SAME:     -> !VPU.DistributedTensor<96x1x1x4xsi32, #NCHW, @CMX_NN,
    // CHECK-sAME:          {mode = "DUPLICATED", num_clusters = 2 : i64, uniform_distributed_segments
    // CHECK-SAME{LITERAL}:  compute_shapes = [[96, 1, 1, 4], [96, 1, 1, 4]]
    // CHECK-SAME{LITERAL}:  compute_offsets = [[0, 0, 0, 0], [0, 0, 0, 0]]
    ...
    // CHECK:       [[CONV_1:%.+]] = VPU.NCE.ClusterTiling (
    // CHECK-SAME:      [[COPY_CONV_INPUT_1]] as [[INNER_ARG23:[^:]+]]: tensor<1x16x79x227xf16, {mem_space = @CMX_NN, order = #NHWC}>,
    // CHECK-SAME:      [[COPY_CONV_WEIGHTS_1]] as [[INNER_ARG24:[^:]+]]: tensor<96x16x7x7xf16, {mem_space = @CMX_NN, order = #NHWC}>,
    // CHECK-SAME:      [[COPY_CONV_WEIGHTS_TBL_1]] as [[INNER_ARG25:[^:]+]]: tensor<96x1x1x4xsi32, {mem_space = @CMX_NN, order = #NCHW}>)
    // CHECK-SAME:     -> !VPU.DistributedTensor<1x96x37x111xf16, #NHWC, @CMX_NN,
    // CHECK-SAME:          {mode = "OVERLAPPED", num_tiles = [1, 1, 2, 1], num_clusters = 2 : i64, uniform_distributed_segments
    // CHECK-SAME{LITERAL}:  compute_shapes = [[1, 96, 19, 111], [1, 96, 18, 111]]
    // CHECK-SAME{LITERAL}:  compute_offsets = [[0, 0, 0, 0], [0, 0, 19, 0]]
    ...
    // CHECK:       [[COPY_CONV_OUT_1:%.+]] = VPU.NCE.ClusterTiling ([[CONV_1]] as [[INNER_ARG26:[^:]+]]: tensor<1x96x37x111xf16, {mem_space = @CMX_NN, order = #NHWC}>)
    // CHECK-SAME:          -> tensor<1x96x37x111xf16, {order = #NHWC}> {
    ...
    // CHECK:       [[COPY_CONV_INPUT_2:%.+]] = VPU.NCE.ClusterTiling ([[SLICE_CONV_2]] as [[INNER_ARG27:[^:]+]]: tensor<1x16x79x227xf16, {order = #NHWC}>)
    // CHECK-SAME:      -> !VPU.DistributedTensor<1x16x79x227xf16, #NHWC, @CMX_NN,
    // CHECK-SAME:           {mode = "OVERLAPPED", num_tiles = [1, 1, 2, 1], num_clusters = 2 : i64, uniform_distributed_segments
    // CHECK-SAME{LITERAL}:   compute_shapes = [[1, 16, 40, 227], [1, 16, 39, 227]]
    // CHECK-SAME{LITERAL}:   compute_offsets = [[0, 0, 0, 0], [0, 0, 40, 0]]
    ...
    // CHECK:       [[COPY_CONV_WEIGHTS_2:%.+]] = VPU.NCE.ClusterTiling ([[WEIGHTS2]] as [[INNER_ARG28:[^:]+]]: tensor<96x16x7x7xf16, {order = #NHWC}>)
    // CHECK-SAME:      -> !VPU.DistributedTensor<96x16x7x7xf16, #NHWC, @CMX_NN,
    // CHECK-SAME:          {mode = "DUPLICATED", num_clusters = 2 : i64, uniform_distributed_segments
    // CHECK-SAME{LITERAL}:  compute_shapes = [[96, 16, 7, 7], [96, 16, 7, 7]]
    // CHECK-SAME{LITERAL}:  compute_offsets = [[0, 0, 0, 0], [0, 0, 0, 0]]
    ...
    // CHECK:       [[COPY_CONV_WEIGHTS_TBL_2:%.+]] = VPU.NCE.ClusterTiling ([[WEIGHTS_TABLE2]] as [[INNER_ARG29:[^:]+]]: tensor<96x1x1x4xsi32>)
    // CHECK-SAME:      -> !VPU.DistributedTensor<96x1x1x4xsi32, #NCHW, @CMX_NN,
    // CHECK-SAME:          {mode = "DUPLICATED", num_clusters = 2 : i64, uniform_distributed_segments
    // CHECK-SAME{LITERAL}:  compute_shapes = [[96, 1, 1, 4], [96, 1, 1, 4]]
    // CHECK-SAME{LITERAL}:  compute_offsets = [[0, 0, 0, 0], [0, 0, 0, 0]]
    ...
    // CHECK-SAME:      [[COPY_CONV_INPUT_2]] as [[INNER_ARG30:[^:]+]]: tensor<1x16x79x227xf16, {mem_space = @CMX_NN, order = #NHWC}>,
    // CHECK-SAME:      [[COPY_CONV_WEIGHTS_2:%.+]] as [[INNER_ARG31:[^:]+]]: tensor<96x16x7x7xf16, {mem_space = @CMX_NN, order = #NHWC}>,
    // CHECK-SAME:      [[COPY_CONV_WEIGHTS_TBL_2:%.+]] as [[INNER_ARG32:[^:]+]]: tensor<96x1x1x4xsi32, {mem_space = @CMX_NN, order = #NCHW}>)
    // CHECK-SAME:     -> !VPU.DistributedTensor<1x96x37x111xf16, #NHWC, @CMX_NN,
    // CHECK-SAME:          {mode = "OVERLAPPED", num_tiles = [1, 1, 2, 1], num_clusters = 2 : i64, uniform_distributed_segments
    // CHECK-SAME{LITERAL}:  compute_shapes = [[1, 96, 19, 111], [1, 96, 18, 111]]
    // CHECK-SAME{LITERAL}:  compute_offsets = [[0, 0, 0, 0], [0, 0, 19, 0]]
    ...
    // CHECK:       [[COPY_CONV_OUT_2:%.+]] = VPU.NCE.ClusterTiling ([[CONV_2]] as [[INNER_ARG33:[^:]+]]: tensor<1x96x37x111xf16, {mem_space = @CMX_NN, order = #NHWC}>)
    // CHECK-SAME:          -> tensor<1x96x37x111xf16, {order = #NHWC}> {
...
func.func @PerAxisQuantizedDWConvIncrementalPipelineCheck(%arg0: tensor<1x32x256x256xf16, {order = #NHWC}>) -> tensor<1x32x256x256x!qElemType, {order = #NHWC}> {
    ...
    // CHECK-DAG:   [[WEIGHTS:%.+]] = const.Declare tensor<32x16x1x1xf16, {order = #NHWC}> = dense<1.000000e+00> : tensor<32x1x1x1xf16>, [#const.Reorder<#NCHW>, #const.Reshape<[32, 1, 1, 1]>, #const.PadWithZero<[0, 0, 0, 0], [0, 15, 0, 0]>, #const.Reorder<#NHWC>]
    ...
    // CHECK:       [[COPY_INPUT_0:%.+]] = VPU.NCE.ClusterTiling ([[SLICE_0]] as [[INNER_ARG0:[^:]+]]: tensor<1x32x52x256xf16, {order = #NHWC}>)
    ...
    // CHECK:           VPU.Copy([[INNER_ARG0]]) {out_mem_space = @CMX_NN} : tensor<1x32x52x256xf16, {order = #NHWC}> -> tensor<1x32x52x256xf16, {mem_space = @CMX_NN, order = #NHWC}>
    // CHECK:           VPU.Yield
    // CHECK:       }
    ...
    // CHECK-SAME:                      [[COPY_INPUT_0]] as [[INNER_ARG3:[^:]+]]: tensor<1x32x52x256xf16, {mem_space = @CMX_NN, order = #NHWC}>,
    ...
    // CHECK-SAME{LITERAL}:  memory_shapes = [[1, 32, 26, 256], [1, 32, 26, 256]], memory_offsets = [[0, 0, 0, 0], [0, 0, 26, 0]]}> {
    // CHECK:           VPU.NCE.DepthConvolution([[INNER_ARG3]], [[INNER_ARG4]], [[INNER_ARG5]])
    // CHECK-SAME:          {pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPETask<mode = <NOOP>, clamp_low = 0 : i64, clamp_high = 255 : i64, lrelu_mult = 1 : i64, lrelu_shift = 0 : i64, fp_prelu_alpha = 1.000000e+00 : f64>, rawFilterShape = [32, 1, 1, 1], strides = [1, 1]} -> tensor<1x32x52x256x!qElemType, {mem_space = @CMX_NN, order = #NHWC}>
    // CHECK:           VPU.Yield
    // CHECK:       }

    // CHECK:       [[COPY_OUTPUT_0:%.+]] = VPU.NCE.ClusterTiling ([[DW_CONV_0]] as [[INNER_ARG6:[^:]+]]: tensor<1x32x52x256x!qElemType, {mem_space = @CMX_NN, order = #NHWC}>) -> tensor<1x32x52x256x!qElemType, {order = #NHWC}> {
    // CHECK:           VPU.Copy([[INNER_ARG6]]) : tensor<1x32x52x256x!qElemType, {mem_space = @CMX_NN, order = #NHWC}> -> tensor<1x32x52x256x!qElemType, {order = #NHWC}>
    // CHECK:           VPU.Yield
    // CHECK:       }
    ...
    // CHECK:       [[COPY_INPUT_1:%.+]] = VPU.NCE.ClusterTiling ([[SLICE_1]] as [[INNER_ARG7:[^:]+]]: tensor<1x32x51x256xf16, {order = #NHWC}>)
    ...
    // CHECK:           VPU.Copy([[INNER_ARG7]]) {out_mem_space = @CMX_NN} : tensor<1x32x51x256xf16, {order = #NHWC}> -> tensor<1x32x51x256xf16, {mem_space = @CMX_NN, order = #NHWC}>
    // CHECK:           VPU.Yield
    // CHECK:       }
    ...
    // CHECK-SAME:                      [[COPY_INPUT_1]] as [[INNER_ARG10:[^:]+]]: tensor<1x32x51x256xf16, {mem_space = @CMX_NN, order = #NHWC}>,
    ...
    // CHECK-SAME{LITERAL}:  memory_shapes = [[1, 32, 26, 256], [1, 32, 25, 256]], memory_offsets = [[0, 0, 0, 0], [0, 0, 26, 0]]}> {
    // CHECK:           VPU.NCE.DepthConvolution([[INNER_ARG10]], [[INNER_ARG11]], [[INNER_ARG12]])
    // CHECK-SAME:          {pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPETask<mode = <NOOP>, clamp_low = 0 : i64, clamp_high = 255 : i64, lrelu_mult = 1 : i64, lrelu_shift = 0 : i64, fp_prelu_alpha = 1.000000e+00 : f64>, rawFilterShape = [32, 1, 1, 1], strides = [1, 1]} -> tensor<1x32x51x256x!qElemType, {mem_space = @CMX_NN, order = #NHWC}>
    // CHECK:           VPU.Yield
    // CHECK:       }

    // CHECK:       [[COPY_OUTPUT_1:%.+]] = VPU.NCE.ClusterTiling ([[DW_CONV_1]] as [[INNER_ARG13:[^:]+]]: tensor<1x32x51x256x!qElemType, {mem_space = @CMX_NN, order = #NHWC}>) -> tensor<1x32x51x256x!qElemType, {order = #NHWC}> {
    // CHECK:           VPU.Copy([[INNER_ARG13]]) : tensor<1x32x51x256x!qElemType, {mem_space = @CMX_NN, order = #NHWC}> -> tensor<1x32x51x256x!qElemType, {order = #NHWC}>
    // CHECK:           VPU.Yield
    // CHECK:       }
    ...
    // CHECK:       [[COPY_INPUT_2:%.+]] = VPU.NCE.ClusterTiling ([[SLICE_2]] as [[INNER_ARG14:[^:]+]]: tensor<1x32x51x256xf16, {order = #NHWC}>)
    // CHECK-SAME:       -> !VPU.DistributedTensor<1x32x51x256xf16, #NHWC, @CMX_NN,
    // CHECK-SAME:           {mode = "OVERLAPPED", num_tiles = [1, 1, 2, 1], num_clusters = 2 : i64, uniform_distributed_segments
    // CHECK-SAME{LITERAL}:  compute_shapes = [[1, 32, 26, 256], [1, 32, 25, 256]], compute_offsets = [[0, 0, 0, 0], [0, 0, 26, 0]]
    // CHECK-SAME{LITERAL}:  memory_shapes = [[1, 32, 26, 256], [1, 32, 25, 256]], memory_offsets = [[0, 0, 0, 0], [0, 0, 26, 0]]}> {
    // CHECK:           VPU.Copy([[INNER_ARG14]]) {out_mem_space = @CMX_NN} : tensor<1x32x51x256xf16, {order = #NHWC}> -> tensor<1x32x51x256xf16, {mem_space = @CMX_NN, order = #NHWC}>
    // CHECK:           VPU.Yield
    // CHECK:       }
    ...
    // CHECK-SAME:                      [[COPY_INPUT_2]] as [[INNER_ARG17:[^:]+]]: tensor<1x32x51x256xf16, {mem_space = @CMX_NN, order = #NHWC}>,
    ...
    // CHECK:       [[COPY_OUTPUT_2:%.+]] = VPU.NCE.ClusterTiling ([[DW_CONV_2]] as [[INNER_ARG20:[^:]+]]: tensor<1x32x51x256x!qElemType, {mem_space = @CMX_NN, order = #NHWC}>) -> tensor<1x32x51x256x!qElemType, {order = #NHWC}> {
    // CHECK:           VPU.Copy([[INNER_ARG20]]) : tensor<1x32x51x256x!qElemType, {mem_space = @CMX_NN, order = #NHWC}> -> tensor<1x32x51x256x!qElemType, {order = #NHWC}>
    // CHECK:           VPU.Yield
    // CHECK:       }
    ...
    // CHECK:       [[COPY_INPUT_3:%.+]] = VPU.NCE.ClusterTiling ([[SLICE_3]] as [[INNER_ARG21:[^:]+]]: tensor<1x32x51x256xf16, {order = #NHWC}>)
    // CHECK-SAME:       -> !VPU.DistributedTensor<1x32x51x256xf16, #NHWC, @CMX_NN,
    // CHECK-SAME:          {mode = "OVERLAPPED", num_tiles = [1, 1, 2, 1], num_clusters = 2 : i64, uniform_distributed_segments
    // CHECK-SAME{LITERAL}:  compute_shapes = [[1, 32, 26, 256], [1, 32, 25, 256]], compute_offsets = [[0, 0, 0, 0], [0, 0, 26, 0]]
    // CHECK-SAME{LITERAL}:  memory_shapes = [[1, 32, 26, 256], [1, 32, 25, 256]], memory_offsets = [[0, 0, 0, 0], [0, 0, 26, 0]]}> {
    // CHECK:           VPU.Copy([[INNER_ARG21]]) {out_mem_space = @CMX_NN} : tensor<1x32x51x256xf16, {order = #NHWC}> -> tensor<1x32x51x256xf16, {mem_space = @CMX_NN, order = #NHWC}>
    // CHECK:           VPU.Yield
    // CHECK:       }
    ...
    // CHECK-SAME:                      [[COPY_INPUT_3]] as [[INNER_ARG24:[^:]+]]: tensor<1x32x51x256xf16, {mem_space = @CMX_NN, order = #NHWC}>,
    ...
    // CHECK:       [[COPY_OUTPUT_3:%.+]] = VPU.NCE.ClusterTiling ([[DW_CONV_3]] as [[INNER_ARG27:[^:]+]]: tensor<1x32x51x256x!qElemType, {mem_space = @CMX_NN, order = #NHWC}>) -> tensor<1x32x51x256x!qElemType, {order = #NHWC}> {
    // CHECK:           VPU.Copy([[INNER_ARG27]]) : tensor<1x32x51x256x!qElemType, {mem_space = @CMX_NN, order = #NHWC}> -> tensor<1x32x51x256x!qElemType, {order = #NHWC}>
    // CHECK:           VPU.Yield
    // CHECK:       }
    ...
    // CHECK:       [[COPY_INPUT_4:%.+]] = VPU.NCE.ClusterTiling ([[SLICE_4]] as [[INNER_ARG21:[^:]+]]: tensor<1x32x51x256xf16, {order = #NHWC}>)
    // CHECK-SAME:       -> !VPU.DistributedTensor<1x32x51x256xf16, #NHWC, @CMX_NN,
    // CHECK-SAME:          {mode = "OVERLAPPED", num_tiles = [1, 1, 2, 1], num_clusters = 2 : i64, uniform_distributed_segments
    // CHECK-SAME{LITERAL}:  compute_shapes = [[1, 32, 26, 256], [1, 32, 25, 256]], compute_offsets = [[0, 0, 0, 0], [0, 0, 26, 0]]
    // CHECK-SAME{LITERAL}:  memory_shapes = [[1, 32, 26, 256], [1, 32, 25, 256]], memory_offsets = [[0, 0, 0, 0], [0, 0, 26, 0]]}> {
    ...
    // CHECK:       [[COPY_WEIGHTS_4:%.+]] = VPU.NCE.ClusterTiling ([[WEIGHTS]] as [[INNER_ARG22:[^:]+]]: tensor<32x16x1x1xf16, {order = #NHWC}>)
    // CHECK-SAME:      -> !VPU.DistributedTensor<32x16x1x1xf16, #NHWC, @CMX_NN,
    // CHECK-SAME:          {mode = "DUPLICATED", num_clusters = 2 : i64, uniform_distributed_segments
    // CHECK-SAME{LITERAL}:  compute_shapes = [[32, 16, 1, 1], [32, 16, 1, 1]], compute_offsets = [[0, 0, 0, 0], [0, 0, 0, 0]]
    // CHECK-SAME{LITERAL}:  memory_shapes = [[32, 16, 1, 1], [32, 16, 1, 1]], memory_offsets = [[0, 0, 0, 0], [0, 0, 0, 0]]}> {
    ...
    // CHECK:       [[COPY_WEIGHTS_TBL_4:%.+]] = VPU.NCE.ClusterTiling ([[WEIGHTS_TABLE]] as [[INNER_ARG23:[^:]+]]: tensor<32x1x1x4xsi32>)
    // CHECK-SAME:      -> !VPU.DistributedTensor<32x1x1x4xsi32, #NCHW, @CMX_NN,
    // CHECK-SAME:          {mode = "DUPLICATED", num_clusters = 2 : i64, uniform_distributed_segments
    // CHECK-SAME{LITERAL}:  compute_shapes = [[32, 1, 1, 4], [32, 1, 1, 4]], compute_offsets = [[0, 0, 0, 0], [0, 0, 0, 0]]
    // CHECK-SAME{LITERAL}:  memory_shapes = [[32, 1, 1, 4], [32, 1, 1, 4]], memory_offsets = [[0, 0, 0, 0], [0, 0, 0, 0]]}> {
    ...
    // CHECK:       [[DW_CONV_4:%.+]] = VPU.NCE.ClusterTiling (
    // CHECK-SAME:                      [[COPY_INPUT_4]] as [[INNER_ARG24:[^:]+]]: tensor<1x32x51x256xf16, {mem_space = @CMX_NN, order = #NHWC}>,
    // CHECK-SAME:                      [[COPY_WEIGHTS_4]] as [[INNER_ARG25:[^:]+]]: tensor<32x16x1x1xf16, {mem_space = @CMX_NN, order = #NHWC}>,
    // CHECK-SAME:                      [[COPY_WEIGHTS_TBL_4]] as [[INNER_ARG26:[^:]+]]: tensor<32x1x1x4xsi32, {mem_space = @CMX_NN, order = #NCHW}>)
    // CHECK-SAME:      -> !VPU.DistributedTensor<1x32x51x256x!qElemType, #NHWC, @CMX_NN,
    // CHECK-SAME:          {mode = "OVERLAPPED", num_tiles = [1, 1, 2, 1], num_clusters = 2 : i64, uniform_distributed_segments
    // CHECK-SAME{LITERAL}:  compute_shapes = [[1, 32, 26, 256], [1, 32, 25, 256]], compute_offsets = [[0, 0, 0, 0], [0, 0, 26, 0]]
    // CHECK-SAME{LITERAL}:  memory_shapes = [[1, 32, 26, 256], [1, 32, 25, 256]], memory_offsets = [[0, 0, 0, 0], [0, 0, 26, 0]]}> {
    ...
    // CHECK:       [[COPY_OUTPUT_4:%.+]] = VPU.NCE.ClusterTiling ([[DW_CONV_4]] as [[INNER_ARG27:[^:]+]]: tensor<1x32x51x256x!qElemType, {mem_space = @CMX_NN, order = #NHWC}>) -> tensor<1x32x51x256x!qElemType, {order = #NHWC}> {
    // CHECK:           VPU.Copy([[INNER_ARG27]]) : tensor<1x32x51x256x!qElemType, {mem_space = @CMX_NN, order = #NHWC}> -> tensor<1x32x51x256x!qElemType, {order = #NHWC}>
    // CHECK:           VPU.Yield
    // CHECK:       }
