...
func.func @CanonicalizeMultipleSlice(%arg0: tensor<1x32x64x64xf16, {order = #NHWC}>)
    ...
    // CHECK-SAME:     [#const.SubView<[64, 0, 0, 0], [64, 32, 3, 3]>, #const.Reorder<#NHWC>, #const.Sparsify<false>]
    // CHECK-DAG:  [[WEIGHTS_SM_SLICE_1:%.*]] = const.Declare tensor<64x1x1x384xi1> =
    // CHECK-SAME:     dense<1.000000e+00> : tensor<128x32x3x3xf16>,
    // CHECK-SAME:     [#const.SubView<[64, 0, 0, 0], [64, 32, 3, 3]>, #const.Reorder<#NHWC>, #const.GetSparsityMap]

    // CHECK-DAG:  [[WEIGHTS_SLICE_0:%.*]] = const.Declare tensor<64x32x3x3xf16, {order = #NHWC}> =
    // CHECK-SAME:     dense<1.000000e+00> : tensor<128x32x3x3xf16>,
    // CHECK-SAME:     [#const.SubView<[0, 0, 0, 0], [64, 32, 3, 3]>, #const.Reorder<#NHWC>, #const.Sparsify<false>]
    // CHECK-DAG:  [[WEIGHTS_SM_SLICE_0:%.*]] = const.Declare tensor<64x1x1x384xi1> =
    // CHECK-SAME:     dense<1.000000e+00> : tensor<128x32x3x3xf16>,
    // CHECK-SAME:     [#const.SubView<[0, 0, 0, 0], [64, 32, 3, 3]>, #const.Reorder<#NHWC>, #const.GetSparsityMap]
