...
func.func @UnrollNCESequence(%input: !Input_DDR, %output: !Output_DDR) -> !Output_DDR {
    ...
    //CHECK:    [[OUT1_CMX_2ND_TASK:%.*]] = VPURT.DeclareBuffer <CMX_NN> [0] <0> -> !VPUIP.ITIBuffer<1x32x32x32xf16, #NHWC, [@CMX_NN, 0]
    //CHECK-SAME:   inwardHaloRegions = [#VPUIP.HaloRegionAttr<shape = [1, 16, 32, 32], offset = [0, 16, 0, 0], cluster_id = 0 : i64>]
    ...
    //CHECK-SAME:      inwardHaloRegions = [#VPUIP.HaloRegionAttr<shape = [1, 16, 32, 32], offset = [0, 0, 0, 0], cluster_id = 1 : i64>]>

    //CHECK:    [[OUT2_CMX_2ND_TASK:%.*]] = VPURT.DeclareBuffer <CMX_NN> [1] <0> -> !VPUIP.ITIBuffer<1x32x32x32xf16, #NHWC, [@CMX_NN, 1]
    //CHECK-SAME:   inwardHaloRegions = [#VPUIP.HaloRegionAttr<shape = [1, 16, 32, 32], offset = [0, 0, 0, 0], cluster_id = 1 : i64>]
    //CHECK-SAME:   outwardHaloRegions = [
    //CHECK-SAME:     #VPUIP.OutwardHaloRegionAttr<
    //CHECK-SAME:      shape = [1, 16, 32, 32]
   ...
   // 2nd task/ 1st subtask
    ...
    //CHECK-SAME:           parent_output([[OUT1_CMX_2ND_TASK]] : !VPUIP.ITIBuffer<1x32x32x32xf16, #NHWC, [@CMX_NN, 0]
    //CHECK-SAME:           outputs([[OUT1_CMX_2ND_TASK]] : !VPUIP.ITIBuffer<1x32x32x32xf16, #NHWC, [@CMX_NN, 0]
    //CHECK-SAME:       variants :  {
    ...
    //CHECK-SAME:           parent_output([[OUT2_CMX_2ND_TASK]] : !VPUIP.ITIBuffer<1x32x32x32xf16, #NHWC, [@CMX_NN, 1]
    //CHECK-SAME:           outputs([[OUT2_CMX_2ND_TASK]] : !VPUIP.ITIBuffer<1x32x32x32xf16, #NHWC, [@CMX_NN, 1]
    //CHECK-SAME:       variants :  {
