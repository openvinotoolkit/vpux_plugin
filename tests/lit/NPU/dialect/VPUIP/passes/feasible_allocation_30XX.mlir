...
!WtMemrefDDR = memref<32x1x1x4xsi32, #NHWC, @DDR>
!ActWinMemrefDDR = memref<1x1x1x32xui8, #NHWC, @DDR>
...
func.func @main(%input: !BufMemrefDDR) -> !BufMemrefDDR {
    %cst0 = const.Declare memref<1x32x64x64xf16, #NHWC, @DDR> = dense<2.0> : tensor<1x32x64x64xf16>, [#const.Reorder<#NHWC>]
    %cst1 = const.Declare !WtMemrefDDR = dense<1> : tensor<32x1x1x4xsi32>, [#const.Reorder<#NHWC>]
    %cst2 = const.Declare !ActWinMemrefDDR = dense<1> : tensor<1x1x1x32xui8>, [#const.Reorder<#NHWC>]
    ...
    %t0, %r0 = async.execute -> !async.value<!BufDistributed> attributes {VPUIP.executor = @DMA_NN, VPUIP.num_units = 1 : i64, "async-deps-index" = 1 : i64} {
        %0 = VPUIP.NNDMA inputs(%cst0 : memref<1x32x64x64xf16, #NHWC, @DDR>) outputs(%buf0 : !BufDistributed) -> !BufDistributed
        async.yield %0: !BufDistributed
    }
    ...
    // CHECK-DAG:       [[CST0:%.*]] = const.Declare memref<1x32x64x64xf16, #NHWC, @DDR>
    // CHECK-DAG:       [[CST1:%.*]] = const.Declare memref<32x1x1x4xsi32, #NHWC, @DDR>
    // CHECK-DAG:       [[CST2:%.*]] = const.Declare memref<1x1x1x32xui8, #NHWC, @DDR>
    ...
    // CHECK:       [[T0:%.*]], [[R0:%.*]] = async.execute
    // CHECK:         VPUIP.NNDMA
    // CHECK-SAME:      inputs(%arg0 : memref<1x32x64x64xf16, #NHWC, @DDR>)
    // CHECK-SAME:      outputs([[BUF0]] : !VPUIP.DistributedBuffer<1x32x64x64xf16, #NHWC, @CMX_NN, {mode = "DUPLICATED", num_clusters = 4 : i64}>)

    // CHECK:       [[T1:%.*]], [[R1:%.*]] = async.execute
    // CHECK-SAME:    [[T0]]
    // CHECK:         VPUIP.NNDMA
    // CHECK-SAME:      inputs([[CST0:.*]] : memref<32x1x1x4xsi32, #NHWC, @DDR>)
    // CHECK-SAME:      outputs([[BUF1]] : !VPUIP.DistributedBuffer<32x1x1x4xsi32, #NHWC, @CMX_NN, {mode = "DUPLICATED", num_clusters = 4 : i64}>)

    // CHECK:       [[T2:%.*]], [[R2:%.*]] = async.execute
    // CHECK-SAME:    [[T0]]
    // CHECK:         VPUIP.NNDMA
    // CHECK-SAME:      inputs([[CST0:.*]] : memref<1x1x1x32xui8, #NHWC, @DDR>)
    // CHECK-SAME:      outputs([[BUF2]] : !VPUIP.DistributedBuffer<1x1x1x32xui8, #NHWC, @CMX_NN, {mode = "DUPLICATED", num_clusters = 4 : i64}>)

    // CHECK:       [[T3:%.*]], [[R3:%.*]] = async.execute
    // CHECK-SAME:    [[T2]]
    // CHECK:         VPUIP.NNDMA
    // CHECK-SAME:       inputs([[CST0:.*]] : memref<1x32x64x64xf16, #NHWC, @DDR>)
    // CHECK-SAME:       outputs([[BUF3]] : !VPUIP.DistributedBuffer<1x32x64x64xf16, #NHWC, @CMX_NN, {mode = "DUPLICATED", num_clusters = 4 : i64}>)

    // CHECK:       [[T4:%.*]], [[R4:%.*]] = async.execute
    // CHECK-SAME:    [[T0]]
    // CHECK:         VPUIP.NNDMA
    // CHECK-SAME:      inputs([[BUF0]] : !VPUIP.DistributedBuffer<1x32x64x64xf16, #NHWC, @CMX_NN, {mode = "DUPLICATED", num_clusters = 4 : i64}>)
    // CHECK-SAME:      outputs([[BUF_SPILL_WRITE]] : memref<1x32x64x64xf16, #NHWC, @DDR>)
    ...
    // CHECK-SAME:         activation_window([[ARG3]]
    // CHECK-SAME:         parent_input([[ARG1]]
    // CHECK-SAME:         parent_output([[BUF4]]
    // CHECK-SAME:         outputs([[BUF4]]
    ...
    // CHECK-SAME:          outputs([[BUF_SPILL_READ]] : !VPUIP.DistributedBuffer<1x32x64x64xf16, #NHWC, @CMX_NN, {mode = "DUPLICATED", num_clusters = 4 : i64}>)
    ...
    // CHECK-SAME:         weights([[ARG2]]
    // CHECK-SAME:         parent_input([[ARG1]]
    // CHECK-SAME:         parent_output([[BUF6]]

    // CHECK:       [[T9:%.*]], [[R9:%.*]] = async.execute
    // CHECK-SAME:    [[T8]]
    // CHECK-SAME:    ([[R8]] as [[ARG1]]: !async.value<!VPUIP.DistributedBuffer<1x32x64x64xf16, #NHWC, @CMX_NN, {mode = "DUPLICATED", num_clusters = 4 : i64}>>)
    // CHECK:             VPUIP.NNDMA
    // CHECK-SAME:          inputs([[ARG1]] : !VPUIP.DistributedBuffer<1x32x64x64xf16, #NHWC, @CMX_NN, {mode = "DUPLICATED", num_clusters = 4 : i64}>)
    // CHECK-SAME:          outputs([[BUF7]] : memref<1x32x64x64xf16, #NHWC, @DDR>)
}
