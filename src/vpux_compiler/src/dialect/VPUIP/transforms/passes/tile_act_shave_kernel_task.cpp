...
bool isSoftmax(VPUIP::SwKernelOp swKernelOp) {
    ...
    return kernelEntryName == "softmax";
}
...
Dim getSwKernelTileDim(VPUIP::SwKernelOp swKernelOp) {
    ...
    if (kernelEntryName == "mvn1") {
        ...
        return Dims4D::Act::H;
    } else if (kernelEntryName == "softmax") {
...
std::optional<OutputTiling> ClusterSwKernelRewriter::calculateOutputTiles(VPUIP::SwKernelOp swKernelOp) const {
    ...
    if (insertSubviewOnly && needAdjustTileSize) {
        ...
        // tile size to make sure the offsets are equal for each cluster. The logic is to find the smallest tile
        // value and change all the tiles' value equal to it except the last one. In this case, the tiles are
        // changed to cluster 0 [1, 8, 1, 1], [1, 9, 1, 1] cluster 1 [1, 8, 1, 1], [1, 8, 1, 1]
        auto iter =
                std::min_element(perClusterShapes.begin(), perClusterShapes.end(), [&](const auto& a, const auto& b) {
                    return a[tileDim] < b[tileDim];
                });
        auto index = std::distance(perClusterShapes.begin(), iter);
        const auto& minOutTile = outTiles[index];
        ...
        for (auto item : outTiles | indexed) {
            ...
            for (auto i : irange(tileSize - 1)) {
                outTilePerCluster[i].shape[tileDim] = minOutTile[i].shape[tileDim];
                outTilePerCluster[i].offsets[tileDim] = minOutTile[i].offsets[tileDim];
            ...
            Shape lastTileOffset = minOutTile.back().offsets;
