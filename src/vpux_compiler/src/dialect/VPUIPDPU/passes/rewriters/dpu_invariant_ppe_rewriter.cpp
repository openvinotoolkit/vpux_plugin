...
mlir::LogicalResult configureIntPPE(const Logger& log, PPEConfig::IntPPE& config, mlir::Type outDataType,
    ...
    if (isQuantizationProvided) {
        ...
        if (DPUInvariantBlockRewriter::getQuantConfig(log, outDataType, quantMult, quantShift, quantZero).failed()) {
            return mlir::failure();
        }
    }
    ...
    if (bypassPPEInt) {
        ...
        if (dpuTaskType == VPUIP::NCETaskType::ELTWISE || dpuTaskType == VPUIP::NCETaskType::AVEPOOL) {
            config.biasAdd.biasStatic = 0;
            config.scaleMult.scaleStatic = quantMult.size() ? quantMult[0] : 1;
            config.scaleShift.shiftStatic = quantShift.size() ? quantShift[0] : 0;
        } else if (dpuTaskType == VPUIP::NCETaskType::MAXPOOL) {
            config.biasAdd.biasStatic = 0;
...
mlir::LogicalResult configurePPE(const Logger& log, PPEConfig& config, mlir::Type inDataType, mlir::Type outDataType,
    ...
    case PPEUseCase::INT_FP:
        if (DPUInvariantBlockRewriter::getBaseType(inDataType).isInteger(CHAR_BIT) &&
            DPUInvariantBlockRewriter::getBaseType(outDataType).isBF16()) {
...
DPUInvariantPPERewriter::DPUInvariantPPERewriter(VPUASM::DPUInvariantOp origInvOp, mlir::Block* invBlock,
...
mlir::LogicalResult DPUInvariantPPERewriter::rewrite() {
    if (!_origInvOp.getPpe().hasOneBlock()) {
        _log.error("VPUASM::DPUInvariant->PPE is not a single block region");
        return mlir::failure();
    }

    if (insertEntryBlock<VPUIPDPU::PPECfgOp>().failed()) {
        return mlir::failure();
    }

    PPEConfig config;
    auto inDataType = getInvBlockArg(DPUInvariantBlockRewriter::BlockArg::ACT_IN)
                              .getType()
                              .cast<mlir::MemRefType>()
                              .getElementType();
    auto outDataType = getInvBlockArg(DPUInvariantBlockRewriter::BlockArg::ACT_OUT)
                               .getType()
                               .cast<mlir::MemRefType>()
                               .getElementType();
    auto dpuTaskType = _origInvOp.getNceTaskType();

    auto ppeTask = evalPPETasks(_log, _origInvOp.getPpe());
    if (ppeTask.first.failed()) {
        return mlir::failure();
    }

    if (configurePPE(_log, config, inDataType, outDataType, dpuTaskType, ppeTask.second).failed()) {
        return mlir::failure();
    }

    if (buildPPEConfig(_rewriter, _origInvOp.getLoc(), _log, config,
                       getInvBlockArg(DPUInvariantBlockRewriter::BlockArg::WEIGHTS_TABLE))
                .failed()) {
        return mlir::failure();
    }
