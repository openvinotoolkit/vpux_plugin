...
vpux::VPU::DistributedTensorNative vpux::VPU::MultiplyOp::getExplicitDistributedTensorAttr(
...
bool VPU::MultiplyOp::doesLayerFitIntoCMX(VPU::MultiClusterStrategy strategy, Byte reservedMem) {
    ...
    auto numClusters = VPU::getOptimalNumClusters(multiplyOp, outputType.getShape(), strategy);

    SmallVector<Byte> buffersSize{
            VPU::getTotalAllocSizeWithDistribution(
                    getInput1().getType(), getActivationDistributionAttrFromOp(multiplyOp, getInput1().getType(),
                                                                               numClusters.getInt(), strategy)),
            VPU::getTotalAllocSizeWithDistribution(
                    getInput2().getType(), getActivationDistributionAttrFromOp(multiplyOp, getInput2().getType(),
                                                                               numClusters.getInt(), strategy)),
            VPU::getTotalAllocSizeWithDistribution(getOutput().getType(),
                                                   getOutputDistributionAttrFromOp(multiplyOp, getOutput().getType(),
                                                                                   numClusters.getInt(), strategy))};
