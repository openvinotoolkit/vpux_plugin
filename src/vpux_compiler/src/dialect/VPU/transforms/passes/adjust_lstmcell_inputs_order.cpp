...
class AdjustLSTMCellInputsOrder final : public mlir::OpRewritePattern<VPU::LSTMCellOp> {
public:
    AdjustLSTMCellInputsOrder(mlir::MLIRContext* ctx, Logger log)
            : mlir::OpRewritePattern<VPU::LSTMCellOp>(ctx), _log(std::move(log)) {
        setDebugName("AdjustLSTMCellInputsOrder");
    }
...
mlir::LogicalResult AdjustLSTMCellInputsOrder::matchAndRewrite(VPU::LSTMCellOp lstmCellOp,
    ...
    VPUX_THROW_WHEN(weightsShape.size() != 4 || weightsShape[2] % 4 != 0, "Invalid weights shape");
    VPUX_THROW_WHEN(recurrenceWeightsShape.size() != 4 || recurrenceWeightsShape[2] % 4 != 0,
                    "Invalid recurrenceWeights shape");
    VPUX_THROW_WHEN(biasesShape.size() != 4 || biasesShape[3] % 4 != 0, "Invaolid biases shape");
    ...
    const auto newWeightsContent = weights.getContentAttr().reshape(newWeightsShape).reorder(newWeightsOrder);
    const auto newRecurrenceWeightsContent =
            recurrenceWeights.getContentAttr().reshape(newRecurrenceWeightsShape).reorder(newWeightsOrder);
    const auto newBiasesConent = biases.getContentAttr().reshape(newBiasesShape).reorder(newBiasesOrder);

    auto newWeightsOp =
            rewriter.create<Const::DeclareOp>(weights.getLoc(), newWeightsContent.getType(), newWeightsContent);
    auto newRecurrenceWeightsOp = rewriter.create<Const::DeclareOp>(
            recurrenceWeights.getLoc(), newRecurrenceWeightsContent.getType(), newRecurrenceWeightsContent);
    auto newBiasesMemPermOp =
            rewriter.create<Const::DeclareOp>(biases.getLoc(), newBiasesConent.getType(), newBiasesConent);
...
class AdjustLSTMCellInputsOrderPass final : public VPU::AdjustLSTMCellInputsOrderBase<AdjustLSTMCellInputsOrderPass> {
public:
    explicit AdjustLSTMCellInputsOrderPass(Logger log) {
...
void AdjustLSTMCellInputsOrderPass::safeRunOnFunc() {
    auto func = getOperation();
    auto& ctx = getContext();

    mlir::RewritePatternSet greedyPatterns(&ctx);
    greedyPatterns.add<AdjustLSTMCellInputsOrder>(&ctx, _log);
...
std::unique_ptr<mlir::Pass> vpux::VPU::createAdjustLSTMCellInputsOrderPass(Logger log) {
    return std::make_unique<AdjustLSTMCellInputsOrderPass>(log);
}
